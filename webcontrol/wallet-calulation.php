...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    $user = $conn->query("SELECT * FROM `members_register` WHERE userid='$userid'")->fetch_assoc();
    ...
    if(isset($_POST['log'])){
        $final = $amount + $bal;
        //update user wallet
        $up = $conn->query("UPDATE `members_register` SET dwallet_balance='$final' WHERE userid='$userid'");
        if($up){
