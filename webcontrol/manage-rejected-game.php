                    ...
                    if (isset($_GET['page_no']) && $_GET['page_no']!="") {
                        ...
                        <div class="col-xs-12">
                            <div class="sec-box">
                                ...
                                <header>
                                    <h2 class="heading">Manage Rejected Game </h2>
                                </header>
                                <div class="contents"  >
                                    ...
                                    <tbody id="myDIV">
                                        ...
                                        $sqls =$conn->query("SELECT * FROM `dbooking_code` INNER JOIN `dgame_categories` ON dgame_categories.category_id = dbooking_code.dodd WHERE dbooking_code.bstatus = 'rejected'");
                                        $total_records =$sqls->num_rows;
                                        $total_no_of_pages = ceil($total_records / $total_records_per_page);
                                        $start_from = ($page_no - 1) * $total_records_per_page;
                                        $v =$conn->query("SELECT * FROM `dbooking_code` INNER JOIN `dgame_categories` ON dgame_categories.category_id = dbooking_code.dodd WHERE dbooking_code.bstatus = 'rejected' ORDER BY dbooking_code.id DESC LIMIT $start_from, $total_records_per_page");
                                        if($v->num_rows>0){
                                        ...
                                        <tr>
                                            ...
                                            <input type="hidden" value="<?php echo basename($_SERVER["REQUEST_URI"]); ?>" id='uri'>
                                                        ...
                                                        <div class="btn-group" >
                                                            ...
                                                            <li><a id="restore" dele="<?php echo $r['booking_id']; ?>" href="#" ><i class="fa fa-reply  text-danger"></i> Restore</a></li>
                                                            <li><a id="dele" dele="<?php echo $r['booking_id']; ?>" href="#" ><i class="fa fa-trash  text-danger"></i> Delete</a></li>
                                                            </ul>
                                                        </div>
                                                        </div>
                                                        </td>
...
<?php
    ...
    $x = $conn->query("SELECT * FROM `dbooking_code` INNER JOIN `dgame_categories` ON dgame_categories.category_id = dbooking_code.dodd WHERE dbooking_code.bstatus = 'rejected' ORDER BY dbooking_code.id DESC LIMIT $start_from, $total_records_per_page");
    if($x->num_rows>0):
