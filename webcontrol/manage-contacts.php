                    ...
                    if (isset($_GET['page_no']) && $_GET['page_no']!="") {
                        ...
                        <div class="col-xs-12">
                            <div class="sec-box">
                                ...
                                <header>
                                    <h2 class="heading">Manage Contact Us</h2>
                                </header>
                                <div class="contents"  >
                                    ...
                                    <section class="boxpadding">
                                        ...
                                        <table class="table table-hover">
                                            ...
                                            <?php 
                                                ...
                                                $sqls = $conn->query("SELECT * FROM `dcontact` ORDER BY id DESC ");
                                                $total_records =$sqls->num_rows;
                                                    $total_no_of_pages = ceil($total_records / $total_records_per_page);
                                                    $start_from = ($page_no - 1) * $total_records_per_page;
                                                
                                                $c = $conn->query("SELECT * FROM `dcontact` ORDER BY id DESC  LIMIT $start_from, $total_records_per_page");
                                                ...
                                                if($c->num_rows>0){
                                                    ...
                                                    <tr style="font-weight:bold">
                                                        ...
                                                        <a class="btn btn-sm btn-primary " id='yes' alert='<?php echo $r['id']; ?>' data-toggle="modal" data-target="#max<?php echo $r['id']; ?>" href="#"><i class="fa fa-edit"></i> Read </a>
                                                        </td>
                                                    </tr>
                                                    <?php }else{?>
                                                        <tr>
...
$(document).ready(function(){
    $(document).on("click", "#yes", function(){
        ...
        $.ajax({
                    url: 'ajax-user.php',
                    type: 'POST',
                    data: {Contact:1,id:id}
                    // success: function(){
                    //     setInterval(function(){
                    //     window.location.assign(mange);
                    //     },2000)
                    // }
                })
    })
        ...
        Swal.fire({
            ...
            if (result.value){
                $.ajax({
                    url: 'ajax-user.php',
                    type: 'POST',
                    data: {ContactD:1,id:id}
                })
                .done(function(response){
                    Swal.fire({type: 'success', title:'Deleted!'});
                    setInterval(function(){
                        window.location.assign('manage-contacts');
                    },2000)
                })
                .fail(function(){
...
<?php
    ...
    $x = $conn->query("SELECT * FROM `dcontact` ORDER BY id DESC  LIMIT $start_from, $total_records_per_page");
    if($x->num_rows>0):
        ...
        <div class="modal-dialog" role="document" style="max-width:550px;">
            <div class="modal-content">
                <form action="manage-odd-update" method="post">
                    ...
                    <h5>Fullname : <?php echo $xx['fullname']; ?></h5>
                    <h5>Email : <?php echo $xx['email']; ?></h5>
                    <h5>Phone : <?php echo $xx['phone']; ?></h5>
                    <P>Message:  <?php echo $xx['text']; ?> </P>

                    </div><!-- End .modal-body -->
                    <div class="modal-footer">
                    <button type="button" class="btn btn-link btn-sm" data-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-danger btn-sm" id="clear" clear="<?php echo $xx['id']; ?>">Delete</button>
                    </div><!-- End .modal-footer -->
                </form>
            </div><!-- End .modal-content -->
        </div><!-- End .modal-dialog -->
