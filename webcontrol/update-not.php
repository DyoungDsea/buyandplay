...
//   if(empty($_FILES['img']['name'])){
        $int = $conn->query("UPDATE `notification` SET dtitle='$name1', ddesc='$name2' WHERE notid='$id' ");
        if($int){
    ...
    //     $int = $conn->query("UPDATE `notifivcation` SET dtitle='$name1', durl='$name3', ddesc='$name2', dimg='$picid' WHERE notid='$id' ");
    //         if($int){
    //             $_SESSION['msgs']="Updated suceessfully";
    //             header("Location:notification");
    //         }else{
