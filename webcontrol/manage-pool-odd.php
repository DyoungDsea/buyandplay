...
<title>Manage Pools Odd : Bet and Buy</title>
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-12">
                            <div class="sec-box">
                                ...
                                <header>
                                <?php 
                                 $cat_id = clean($_GET['odd_id']);
                                 $cd = $conn->query("SELECT * FROM `dpool_general` WHERE dstatus !='Free' AND dcat_id='$cat_id' GROUP BY dcat_id ORDER BY dcategory")->fetch_assoc();
                                 $cate = $cd['dcategory'];
                                 ?>
                                    <h2 class="heading">Manage <?php echo $cate; ?></h2>
                                </header>
                                <div class="contents"  >
                                    ...
                                    <section class="boxpadding">
                                        ...
                                        <table class="table table-hover">
                                            ...
                                            <?php 
                                                ...
                                                $cat_id = clean($_GET['odd_id']);
                                                $c = $conn->query("SELECT * FROM `dpool_general` WHERE dcat_id='$cat_id' AND dstatus !='Free'");
        ...
        <div class="modal-dialog" role="document" style="max-width:550px;">
            <div class="modal-content">
                <form action="manage-ranking-process" method="post">
                    ...
                    <button type="button" class="btn btn-link btn-sm" data-dismiss="modal">Colse</button>
                        <button type="submit" name="ten" class="btn btn-primary btn-sm">Submit</button>
...
<?php
    ...
    $x = $conn->query("SELECT * FROM `dpool_general` WHERE dcat_id='$cat_id' AND dstatus !='Free'");
    if($x->num_rows>0):
        ...
        <div class="modal-dialog" role="document" style="max-width:550px;">
            <div class="modal-content">
                <form action="manage-odd-update" method="post">
                    <div class="modal-header">
                        <h3 class="modal-title" id="addressModalLabel">Update Manage <?php echo $xx['dcategory']; ?></h3>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                        </button>
                    </div><!-- End .modal-header -->
                    <div class="modal-body">
                            <div class="form-group">
                                ...
                                <input type="hidden" name="uri"  value="manage-pool-odd?odd_id=<?php echo $_GET['odd_id'] ?>&category=<?php echo $_GET['category'] ?>">
                            </div>
                            <div class="form-group">
                    ...
                    <button type="button" class="btn btn-link btn-sm" data-dismiss="modal">Close</button>
                        <button type="submit" name="nap" class="btn btn-primary btn-sm">Update</button>
                    </div><!-- End .modal-footer -->
                </form>
            </div><!-- End .modal-content -->
        </div><!-- End .modal-dialog -->
