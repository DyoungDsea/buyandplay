                    ...
                    if (isset($_GET['page_no']) && $_GET['page_no']!="") {
                        ...
                        <div class="col-xs-12">
                            <div class="sec-box">
                                ...
                                <header>
                                    <h2 class="heading"> Cancelled Request</h2>
                                </header>
                                <div class="contents"  >
                                    ...
                                    <section class="boxpadding">
                                        ...
                                        <table class="table table-hover">
                                            ...
                                            <?php 
                                                ...
                                                $sqls=$conn->query("SELECT * FROM `dwithdrawal_history` WHERE withdrawal_status='cancelled' ORDER BY id");
                                                $total_records =$sqls->num_rows;
                                                $total_no_of_pages = ceil($total_records / $total_records_per_page);
                                                $start_from = ($page_no - 1) * $total_records_per_page;
                                                // $c = $conn->query("SELECT * FROM `dcategory_subscriptions` WHERE dstatus='cancelled' ORDER BY dcategory LIMIT $start_from, $total_records_per_page");

                                                $c = $conn->query("SELECT * FROM `dwithdrawal_history` INNER JOIN `admin_accounts` ON    dwithdrawal_history.user_id = admin_accounts.userid   WHERE dwithdrawal_history.withdrawal_status='cancelled' ORDER BY dwithdrawal_history.pdate DESC");
