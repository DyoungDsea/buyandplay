...
<head>
  ...
  <title>Buy and Bet | VIP Process </title>
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        if(isset($_SESSION['name']) && isset($_SESSION['fee']) && isset($_SESSION['id'])){
            $cid = $_SESSION['id'];
        $d = $conn->query("SELECT * FROM `dvip_categories` WHERE vip_id='$cid'");
        if($d->num_rows>0):
            $r=$d->fetch_assoc()?>
                <div class="col-md-6">
                    <div class="card text-center">
                        ...
                        <?php 
                          $idz = clean($_SESSION['userId']);
                        $boo = $conn->query("SELECT dvip FROM `members_register`  WHERE userid='$idz' AND dvip='active'");
                        if($boo->num_rows>0){?>
                          <p>Do you want to extend your VIP subscription with </p> 
                        <?php  }else{?>
                          <p>Do you want to become a VIP for </p> 
                        <?php  }

                        ?>                       
                        <h5><?php echo $r['dmonth']; ?></h5>
                        <h6 class="mt-2">Cost: <?php echo formatNaira($r['dprice']) ?></h6>

                        <form action="vip-cart" method="post">                                       
                        <hr>
