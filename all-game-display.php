          ...
          if($x->num_rows>0):            
            while($r=$x->fetch_assoc()):
                ...
                if($q['dstatus'] =='gold' AND $q['dcat_id']==$r['dodd']){
                    $ran = $conn->query("SELECT * FROM `ranking` WHERE dstar='5' ORDER BY dstar DESC "); 
                    $ra = $ran->fetch_assoc();
                  ...
                  <div class="card mb-4">
                    ...
                    $vp = $conn->query("SELECT username FROM `members_register` WHERE userid='$uvp'")->fetch_assoc(); ?>
                      <p class="mt-1">Forecatser:  <?php echo $vp['username']; ?></p>
                      ...
                      <?php  }else{
                          ...
                          if($dmarket->num_rows>0){ ?> 
                            <a href="view-game?booking=<?php echo $r['booking_id'] ?>" class="btn btn-secondary btn-sm mb-5m"> View Free </a>
                          <?php   }else{
                        ?>
                    ...
                    $ran = $conn->query("SELECT * FROM `ranking` WHERE dstar='4' ORDER BY dstar DESC"); 
                    $ra = $ran->fetch_assoc();
                    $price = formatNaira($ra['dprice']);
                    if($r['dtotal'] == $won ):?>
                        <div class="col-md-3">
                  <div class="card mb-4">
                    ...
                    $ran = $conn->query("SELECT * FROM `ranking` WHERE dstar='3'  ORDER BY dstar DESC"); 
                    $ra = $ran->fetch_assoc();
                    $price = formatNaira($ra['dprice']);
                    if($r['dtotal'] == $won ):?>
                        <div class="col-md-3">
