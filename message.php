  ...
  <section class="blog-details-section mt-5">
    <div class="container-fluid">
      <div class="row">
        ...
        <div class="boxer">
                <?php $u = clean($_SESSION['userId']); $h = $conn->query("SELECT * FROM dmessaging WHERE dreceiver ='$u' AND dstatus='unread' ") ?>
                            <div class="card text-center mb-3 p-4">
                                <h4> Messages inbox (<?php echo $h->num_rows; ?>)</h4>
                            </div>
        </div>
        </div>
        ...
        <div class="col-md-4">
        <div class="frm-group">
            <a href="compose" style="font-size:16px" class="btn hide-small btn-primary"> Compose message </a>
        </div>
        </div>
        </div>
            <div class="table-responsive">
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        $sqls =$conn->query("SELECT * FROM `dmessaging` WHERE dreceiver='$id' AND dstatus !='delete' ORDER BY id DESC");
                        $total_records =$sqls->num_rows;
                        $total_no_of_pages = ceil($total_records / $total_records_per_page);
                        $start_from = ($page_no - 1) * $total_records_per_page;
                        $v =$conn->query("SELECT * FROM `dmessaging` WHERE dreceiver='$id' AND dstatus !='delete' ORDER BY id DESC LIMIT $start_from, $total_records_per_page");
                        if($v->num_rows>0){
                        ...
                        <tr style="font-weight:bold">
                            ...
                            <td><?php echo limit_text($r['dmessage'],5) ?></td>
                            <td><?php echo date("d-M-Y h:i:sa", strtotime($r['dtime'])) ?></td>
                            <td>
                            <div class="btn-group">
                                <div class="btn-group" >
                                    ...
                                    <li><a class="nav-link" href="read-message?id=<?php echo $r['transid']; ?>">Read</a></li>
                                    <li><a class="nav-link" id="smsRemove" trans="<?php echo $r['transid']; ?>" href="#">Delete</a></li>
                                    </ul>
                                </div>
                                </div>
                            </td>
                        ...
                        <tr >
                            ...
                            <div class="btn-group">
                                <div class="btn-group" >
                                    ...
                                    Action </button>
                                    <ul class="dropdown-menu" role="menu" style="font-size:14px; width:100%;text-align:center">
                                    <li><a class="nav-link" href="read-message?id=<?php echo $r['transid']; ?>&sender=<?php echo $r['dsender']; ?>">Read</a></li>
                                    <li><a class="nav-link" id="smsRemove" trans="<?php echo $r['transid']; ?>" href="#">Delete</a></li>
                                    </ul>
                                </div>
                                </div>
